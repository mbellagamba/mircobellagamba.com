...
function applyColorScheme(value) {
	...
	getToggleButton().innerHTML = value === DARK_COLOR_SCHEME ? DARK_SVG_ICON : LIGHT_SVG_ICON;
	setCookie(COLOR_SCHEME_KEY, value, THEME_EXPIRATION_IN_DAYS)
}

function toggleColorScheme() {
	const isDarkColorScheme = getColorScheme() === DARK_COLOR_SCHEME;
	const newTheme = isDarkColorScheme ? LIGHT_COLOR_SCHEME : DARK_COLOR_SCHEME;
	applyColorScheme(newTheme);
}

function initColorSchemeToggle() {
	getToggleButton().addEventListener("click", toggleColorScheme);
}

function initializeColorScheme() {
	const darkMediaQuery = window.matchMedia('(prefers-color-scheme: dark)');
	const mqColorScheme = (mq) => mq.matches ? DARK_COLOR_SCHEME : LIGHT_COLOR_SCHEME
	const onDarkPreferenceChange = function (ev) {
		applyColorScheme(mqColorScheme(ev));
	}
	try {
		darkMediaQuery.addEventListener('change', onDarkPreferenceChange);
	} catch {
		darkMediaQuery.addListener(onDarkPreferenceChange);
	}

	const colorScheme = getColorScheme() || mqColorScheme(darkMediaQuery);
